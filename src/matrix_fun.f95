...
! коэффициенты 3х-диагонал матрицы по Ходжу (begin)
...
! коэффициенты 3х-диагонал матрицы по Ходжу (end)


! составляющие коэффициентов преобразованный 3х-диагонал матрицы по Ходжу (begin)
function D_q(q, n, m, c) result(f)
...
! составляющие коэффициентов преобразованный 3х-диагонал матрицы по Ходжу (end)


! изначальная 3х-диагональная матрица в полном виде по Ходжу
function ABC_matr_old(NN, n, m, c) result(f)
...
! изначальная 3х-диагональная матрица в полном виде по Ходжу
function ABC_matr(NN, n, m, c) result(f)
	integer, intent(in) :: n, m, NN
	integer :: r, s, k
	complex(knd), intent(in) :: c
	complex(knd) :: f(0:NN-1,0:NN-1)
...
function DFE_matr(NN, n, m, c) result(f)
	integer, intent(in) :: n, m, NN
	integer :: r, s
	complex(knd), intent(in) :: c
	complex(knd) :: f(0:NN-1,0:NN-1)
	...
	do r=0, (NN-1)-1
		f(r,r+1) = sqrt(D_q(r+1, n, m, c) * F_q(r, n, m, c))
		f(r,r) = E_q(r, n, m, c)
		f(r+1,r) = f(r,r+1)
	end do
	r = NN-1
...
! симметричная (преобразованная) 3х-диагональная матрица в полном виде по Ходжу
function DFE_matr_old(NN, n, m, c) result(f)
	integer, intent(in) :: n, m, NN
	integer :: r, s
	complex(knd), intent(in) :: c
	complex(knd) :: f(NN,NN)
	...
	r = 1
	! f(r,r-1) = sqrt(D_q(r, n, m, c) * F_q(r-1, n, m, c))
	f(r,r) = E_q(r, n, m, c)
	f(r,r+1) = sqrt(D_q(r+1, n, m, c) * F_q(r, n, m, c))
...
function DFE_matr_mini(NN, n, m, c) result(f)
	integer, intent(in)			:: n, m, NN
	complex(knd), intent(in)	:: c
	...
	do r=0, NN-1
		f(r+1,1) = E_q(r, n, m, c)
		f(r+1,2) = sqrt(D_q(r+1, n, m, c) * F_q(r, n, m, c))
	end do
...
function DFE_matr_mini_full(NN, n, m, c) result(f)
	integer, intent(in)			:: n, m, NN
	integer						:: r, s
	complex(knd), intent(in)	:: c
...
! коэффициенты 3х-диагонал матрицы по Баукампу (begin)
...
! коэффициенты 3х-диагонал матрицы по Баукампу (end)

function ABC_matr_b(NN, n, m, c) result(f)
	integer, intent(in) :: n, m, NN
	integer :: r, s, k
	complex(knd), intent(in) :: c
	complex(knd) :: f(0:NN-1,0:NN-1)
...
function DFE_matr_b(NN, n, m, c) result(f)
	integer, intent(in)			:: n, m, NN
	integer						:: r, s, k
	complex(knd), intent(in)	:: c
	complex(knd)				:: f(0:NN-1,0:NN-1)
	...
	do r=0, (NN-1)-1
		f(r,r+1) = sqrt(D_q_b(r+1, n, m, c) * F_q_b(r, n, m, c))
		f(r,r) = E_q_b(r, n, m, c)
		f(r+1,r) = f(r,r+1)
	end do
...
function DFE_matr_mini_b(NN, n, m, c) result(f)
	integer, intent(in)			:: n, m, NN
	integer						:: r, s
	complex(knd), intent(in)	:: c
	complex(knd)				:: f(NN,2)
	...
	do r=0, NN-1
		f(r+1,1) = E_q_b(r, n, m, c)
		f(r+1,2) = sqrt(D_q_b(r+1, n, m, c) * F_q_b(r, n, m, c))
	end do
...
function DFE_matr_mini_b_full(NN, n, m, c) result(f)
	integer, intent(in)			:: n, m, NN
	integer						:: r, s
	complex(knd), intent(in)	:: c
	complex(knd)				:: f(NN/2,4)
	...
	f(:,:2) = DFE_matr_mini_b(NN/2, n, m, c)
...
function D_q_b(q, n, m, c) result(f)
	integer, intent(in) :: n, m, q
	complex(knd), intent(in) :: c
	complex(knd) :: f
...
function E_q_b(q, n, m, c) result(f)
	integer, intent(in) :: n, m, q
	complex(knd), intent(in) :: c
	complex(knd) :: f
...
function F_q_b(q, n, m, c) result(f)
	integer, intent(in) :: n, m, q
	complex(knd), intent(in) :: c
	complex(knd) :: f
